...
public class AutoTypePasswordPicker extends BorderPane {
    // needed since switching apps requires Meta+Tab on mac,
    // and Alt+Tab on windows
    private static final int SWITCH_APPS_KEY   = System.getProperty("os.name").toLowerCase().contains("mac") ?
            KeyEvent.VK_META : KeyEvent.VK_ALT;
    ...
    // todo remove, as using awt robot pulls in entire
    // AWT runtime. Instead, use either GlobalScreen.postNativeEvent or com.sun.glass.ui.Robot
    // Also note that the AWT robot is very temperrmental; the constants set were determined
    // by trial and error on my Mac, and may not work well on other devices.
    private Robot robot;

    public AutoTypePasswordPicker(KdbxContext ctx) {
        this.context = ctx;

        FXMLLoader fxml = new FXMLLoader(getClass().getResource("/views/autotype-password-picker.fxml"));
        fxml.setRoot(this);
        ...
        filteredEntries = new FilteredList<>(context.entries);

        entryListView.setCellFactory(listview -> new EntryModelListCell());
        entryListView.setItems(filteredEntries);
        entryListView.getSelectionModel().selectedItemProperty().addListener(this::selectEntry);

        search.textProperty().addListener((obs, oldSearch, newSearch) -> {
            if (newSearch == null) filteredEntries.setPredicate(null);
            else SearchEngine.filter(newSearch, filteredEntries);
        });

    }

    private void selectEntry(ObservableValue obs, EntryModel oldSelection, EntryModel newEntry) {
        if (newEntry == null || newEntry.usernameProperty().isEmpty().get()
                || newEntry.passwordProperty().isEmpty().get()) return;

        String userName = newEntry.usernameProperty().get();
        String password = newEntry.passwordProperty().get();

        getScene().getWindow().setOnHidden(we -> {
            switchTabs();
            for (char c : userName.toCharArray()) type(c);
            type('\t');
            for (char c : password.toCharArray()) type(c);
    ...
    private void switchTabs() {
        robot.keyPress(SWITCH_APPS_KEY);
        // todo fix these random constants when
        // JavaFX Robot is made public
        robot.delay(12);

        robot.keyPress(KeyEvent.VK_TAB);
        robot.keyRelease(KeyEvent.VK_TAB);
        robot.keyRelease(SWITCH_APPS_KEY);
